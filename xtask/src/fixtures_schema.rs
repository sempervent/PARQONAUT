//! Reproducibly generate FRANKENLAKE v2 and supporting schema fixtures.
//!
//! Usage: cargo run -p parqonaut-repair --bin xtask (fixtures schema) [output_root]
...
pub fn run(root: &camino::Utf8Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("schema fixtures written under {}", root.display());
    Ok(())
}
