//! Generate FOGBANK fixtures (local Parquet + optional MinIO upload).
//!
//! ```text
//! cargo run -p parqonaut-storage --features s3 --bin xtask (fixtures object-storage) -- fixtures/object-storage/local
//! cargo run -p parqonaut-storage --features s3 --bin xtask (fixtures object-storage) -- fixtures/object-storage/local --upload
//! ```
...
pub fn run(root: &camino::Utf8Path) -> Result<(), Box<dyn std::error::Error>> {
    let upload = std::env::var("PARQONAUT_XTASK_UPLOAD").ok().as_deref() == Some("1");
    let root = root.as_std_path();
    fs::create_dir_all(root)?;
    let manifest_root = root
        .parent()
        .map(|p| p.join("manifests"))
        .unwrap_or_else(|| PathBuf::from("fixtures/object-storage/manifests"));
    ...
    println!("FOGBANK local fixtures written under {}", root.display());
    println!();
    ...
    if upload {
        ...
        rt.block_on(upload_fixtures(root, manifest_root.as_path()))?;
    } else {
        println!();
        println!("Local-only generation complete. Set PARQONAUT_XTASK_UPLOAD=1 to upload to S3.");
    }
...
```bash
cp fixtures/object-storage/local/stale-source/snapshot-02.parquet \
   fixtures/object-storage/local/stale-source/snapshot-03.parquet
cargo run -p parqonaut-storage --features s3 --bin xtask (fixtures object-storage) -- \
  fixtures/object-storage/local --upload
```
