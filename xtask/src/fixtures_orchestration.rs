//! Reproducibly generate SHIPWRECK batch-orchestration fixture fleet.
//!
//! Usage: cargo run -p parqonaut-orchestrator --bin xtask (fixtures orchestration) [output_root]
...
pub fn run(root: &camino::Utf8Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("SHIPWRECK fixtures written under {}", root.display());
    println!();
