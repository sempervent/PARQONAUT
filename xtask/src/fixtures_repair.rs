//! Reproducibly generate repair-pathology Parquet fixtures.
//!
//! Usage: cargo run -p parqonaut-repair --bin xtask (fixtures repair) [output_root]
...
pub fn run(root: &camino::Utf8Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("repair fixtures written under {}", root.display());
    Ok(())
}
