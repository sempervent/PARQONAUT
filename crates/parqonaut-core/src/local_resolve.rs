...
use parqonaut_types::{ScanOptions, ScanPlan, ScanTarget};
use walkdir::WalkDir;
...
fn resolve_file(path: &Utf8Path, _options: &ScanOptions) -> Result<ScanPlan, CoreError> {
    ...
    Ok(ScanPlan {
        root,
        assets: vec![parqonaut_types::ResolvedAsset {
            path: path.to_owned(),
            format,
            size_bytes: meta.len(),
        }],
...
fn resolve_directory(root: &Utf8Path, options: &ScanOptions) -> Result<ScanPlan, CoreError> {
        ...
        WalkDir::new(root.as_std_path()).follow_links(false).into_iter().filter_map(|e| e.ok())
    {
        ...
        assets.push(parqonaut_types::ResolvedAsset { path: utf8, format, size_bytes: meta.len() });
        if assets.len() >= cap {
            truncated = true;
            break;
        }
...
mod tests {
    ...
    use parqonaut_types::{ScanOptions, ScanTarget};
    ...
    fn resolves_single_file() {
        let root =
            Utf8PathBuf::from_path_buf(std::env::temp_dir().join("parqonaut_resolve_single"))
                .unwrap();
