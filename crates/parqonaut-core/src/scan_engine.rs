...
use parqonaut_types::{PluginExecutionRecord, ScanPluginMetadata};
...
impl ScanEngine {
    ...
    pub fn run_with_plugins(
        ...
        let mut pre_scan_findings: Vec<parqonaut_types::Finding> = Vec::new();
...
fn finding_summaries(findings: &[parqonaut_types::Finding]) -> Vec<serde_json::Value> {
    findings
        .iter()
        .map(|f| {
            serde_json::json!({
...
fn evidence_id_set(findings: &[parqonaut_types::Finding]) -> BTreeSet<String> {
