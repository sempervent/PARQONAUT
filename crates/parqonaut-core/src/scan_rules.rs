//! Built-in scan quality rules (namespaced finding codes).
...
/// Evaluates built-in scan rules against resolved assets and Parquet inspections.
pub fn evaluate_scan_rules(
    plan: &ScanPlan,
    ...
    let unknown: Vec<&ResolvedAsset> =
        assets.iter().filter(|a| a.format == parqonaut_types::DataFormat::Unknown).collect();
    if !unknown.is_empty() {
    ...
    for a in assets {
        match a.format {
            parqonaut_types::DataFormat::Unknown => {}
            other => {
    ...
    for a in assets {
        if a.format == parqonaut_types::DataFormat::Parquet && a.size_bytes < FILE_TOO_SMALL_BYTES {
            out.push(finalize(Finding {
                id: Uuid::new_v4(),
...
fn format_label(f: &parqonaut_types::DataFormat) -> &'static str {
    match f {
        parqonaut_types::DataFormat::Parquet => "parquet",
        parqonaut_types::DataFormat::Csv => "csv",
        parqonaut_types::DataFormat::Json => "json",
        parqonaut_types::DataFormat::Ndjson => "ndjson",
        parqonaut_types::DataFormat::Unknown => "unknown",
    }
}
