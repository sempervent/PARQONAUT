...
use parqonaut_types::{system, InspectionStatus, ScanProfile, ScanRequest, ScanTarget};
use uuid::Uuid;

use parqonaut_core::{resolve_local_scan_plan, ScanEngine};
...
fn codes(report: &parqonaut_types::ScanReport) -> Vec<String> {
...
fn scan_single_parquet_produces_valid_report() {
    ...
    parqonaut_types::validate_report(&report).unwrap();
...
fn unknown_extension_emits_format_unknown() {
    let dir = std::env::temp_dir().join(format!("parqonaut_unknown_{}", Uuid::new_v4()));
...
fn validate(report: &parqonaut_types::ScanReport) {
    parqonaut_types::validate_report(report).unwrap();
}
...
fn local_scan_plan_counts_parquet_files() {
    ...
    let plan =
        resolve_local_scan_plan(&target, &parqonaut_types::ScanOptions::default()).expect("plan");
...
fn single_parquet_scan_json_roundtrips() {
    ...
    let back: parqonaut_types::ScanReport = serde_json::from_value(v.clone()).unwrap();
    parqonaut_types::validate_report(&back).unwrap();
...
fn truncation_emits_scan_truncated_finding() {
    let dir = std::env::temp_dir().join(format!("parqonaut_trunc_{}", Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    for i in 0..5 {
