...
use parqonaut_types::{Dataset, Finding, ScanTarget};
...
impl RuleEngine for EmptyRuleEngine {
    fn evaluate(&self, _ctx: &parqonaut_core::RuleScanContext) -> Result<Vec<Finding>, CoreError> {
        Ok(Vec::new())
    }
}
...
fn orchestrator_classifies_parquet_by_extension() {
    let o = orchestrator();
    let req = parqonaut_types::ScanRequest::new(
        ScanTarget::LocalFile { path: Utf8PathBuf::from("data/example.parquet") },
        parqonaut_types::ScanProfile::Quick,
    );
    let formats = o.classify_formats(&req).expect("formats");
    assert_eq!(formats, vec![parqonaut_types::DataFormat::Parquet]);
}
...
fn orchestrator_runs_rules_with_empty_engine() {
    ...
    let req = parqonaut_types::ScanRequest::new(
        ...
        parqonaut_types::ScanProfile::Standard,
    );
