//! In-process background worker: claims queued application jobs and delegates to [`ParacleteService`].
...
use paraclete_types::{JobId, JobKind, JobRecoveryPolicy, RunId};
...
pub(crate) fn spawn_scan_job_workers(
    service: ParacleteService,
    workers: usize,
) -> Vec<tokio::task::JoinHandle<()>> {
    (0..workers).map(|_| spawn_application_job_worker(service.clone())).collect()
}

fn spawn_application_job_worker(service: ParacleteService) -> tokio::task::JoinHandle<()> {
...
async fn cancel_running_job(service: &ParacleteService, jid: JobId) -> Result<(), AppError> {
...
async fn fail_job(service: &ParacleteService, jid: JobId, e: &AppError) {
