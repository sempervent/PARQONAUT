...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // safe-only
    let small = root.join("fixtures/phase2/small-files");
    if small.exists() {
    ...
    let franken = root.join("fixtures/phase3/frankenlake-v2");
    let policy_toml = fs::read_to_string(root.join("fixtures/phase3/policy.toml"))?;
    ...
    let rename = root.join("fixtures/phase3/rename-map");
    let rename_policy = fs::read_to_string(root.join("fixtures/phase3/rename-policy.toml"))?;
    ...
    // unresolvable
    let explicit = root.join("fixtures/phase3/explicit-target");
