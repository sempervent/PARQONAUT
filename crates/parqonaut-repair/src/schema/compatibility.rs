...
/// Loss classification for a type conversion.
...
/// Classify conversion from `from` to `to` under schema policy.
pub fn classify_conversion(
    ...
    if from == to && !from_nullable && to_nullable && policy.allow_nullable_widening {
        return Compatibility::Lossless;
    }

    if from != to {
        if !policy.allow_numeric_widening {
        ...
        match (from, to) {
            ...
            | (PhysicalType::Float32, PhysicalType::Float64) => {
                if from_nullable == to_nullable
                    || (!from_nullable && to_nullable && policy.allow_nullable_widening)
                {
                    return Compatibility::Lossless;
                }
