...
fn ensure_schema_fixtures() {
    ...
    let status = StdCommand::new("cargo")
        ...
        .status()
        .expect("generate schema fixtures");
...
fn plan_json(root: &std::path::Path) -> serde_json::Value {
    ...
    Command::cargo_bin("prqnt")
        ...
        .args([
            ...
            root.to_str().unwrap(),
            "--policy",
            workspace_root().join("fixtures/schema/policy.toml").to_str().unwrap(),
            "--output",
...
fn unauthorized_review_required_skipped() {
    ...
    let out = workspace_root().join("target/test-schema-unauth");
...
fn frankenlake_v2_authorized_repair_and_verify() {
    ...
    let source = root.join("fixtures/schema/frankenlake-v2");
    ...
    Command::cargo_bin("prqnt")
        ...
        .args([
            ...
            "--policy",
            root.join("fixtures/schema/policy.toml").to_str().unwrap(),
            "--output",
    ...
    let mut repair_cmd = Command::cargo_bin("prqnt").unwrap();
    repair_cmd.args([
        "repair",
        source.to_str().unwrap(),
        "--plan",
    ...
    Command::cargo_bin("prqnt")
        ...
        .args([
            ...
            out.to_str().unwrap(),
            "--policy",
            root.join("fixtures/schema/ci-policy.toml").to_str().unwrap(),
        ])
        .assert()
        .code(4);
}
