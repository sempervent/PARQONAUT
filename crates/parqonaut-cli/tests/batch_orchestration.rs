...
fn shipwreck_config() -> camino::Utf8PathBuf {
    repo_root().join("fixtures/orchestration/shipwreck/batch.toml")
}

fn shipwreck_root() -> camino::Utf8PathBuf {
    repo_root().join("fixtures/orchestration/shipwreck")
}

fn generate_fixtures() {
    let status = Command::new("cargo")
        .args(["xtask", "fixtures", "orchestration"])
...
fn run_batch(args: &[&str]) -> assert_cmd::assert::Assert {
    let mut cmd = cargo_bin_cmd!("prqnt");
...
fn terminal_state_map(dir: &Path) -> HashMap<String, String> {
    ...
    let status = Command::new(cargo_bin_cmd!("prqnt").get_program())
...
fn status_and_verify_from_persisted_journal() {
    ...
    let status1 = Command::new(cargo_bin_cmd!("prqnt").get_program())
        .args(["batch", "status", "--run-dir", run_dir_str, "--json"])
        .current_dir(repo_root().as_std_path())
        .output()
        .unwrap();
    assert!(status1.status.success());
    let status2 = Command::new(cargo_bin_cmd!("prqnt").get_program())
        .env_clear()
        .args(["batch", "status", "--run-dir", run_dir_str, "--json"])
        .current_dir(repo_root().as_std_path())
        .output()
    ...
    let verify1 = Command::new(cargo_bin_cmd!("prqnt").get_program())
