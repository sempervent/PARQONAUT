...
fn ensure_repair_fixtures() {
    let root = workspace_root().join("fixtures/repair/frankenlake");
    if root.exists() {
        return;
    }
...
fn frankenlake_doctor_diagnosis() {
    ensure_repair_fixtures();
    let path = workspace_root().join("fixtures/repair/frankenlake");
...
fn frankenlake_repair_and_verify() {
    ...
    let source = root.join("fixtures/repair/frankenlake");
...
fn stale_plan_rejected() {
    ...
    for entry in std::fs::read_dir(workspace_root().join("fixtures/repair/small-files")).unwrap() {
