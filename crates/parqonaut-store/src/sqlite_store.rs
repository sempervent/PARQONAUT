...
impl SqliteScanStore {
    ...
    /// Inserts a queued application job (HTTP submission path).
    pub async fn insert_application_job(
        &self,
        job_id: JobId,
        kind: parqonaut_types::JobKind,
        identity: &TargetIdentity,
        request_json: &str,
    ) -> Result<(), StoreError> {
    ...
    pub async fn insert_scan_job_queued(
        ...
        self.insert_application_job(job_id, parqonaut_types::JobKind::Scan, identity, request_json)
            .await
    }
    ...
    pub async fn complete_application_job_success(
        ...
        let res = sqlx::query(
            r#"UPDATE application_jobs SET status = 'succeeded', completed_at = ?, run_id = ?,
               result_ref_json = ?, worker_id = NULL, heartbeat_at = NULL, leased_until = NULL,
               cancel_requested = 0
               WHERE job_id = ? AND status = 'running'"#,
        )
        .bind(&completed)
    ...
    pub async fn complete_application_job_canceled(&self, job_id: JobId) -> Result<(), StoreError> {
        let completed = Utc::now().to_rfc3339();
        let res = sqlx::query(
            r#"UPDATE application_jobs SET status = 'canceled', completed_at = ?,
...
mod tests {
    ...
    use parqonaut_types::{JobId, JobRecoveryPolicy};
    use uuid::Uuid;
    ...
    async fn scan_job_success_links_existing_run() {
        use parqonaut_types::ScanRequest;
        ...
        let target = parqonaut_types::ScanTarget::LocalFile { path };
