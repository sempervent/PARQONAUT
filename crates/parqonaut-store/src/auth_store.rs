...
use parqonaut_types::{AuthPrincipal, AuthRole, AuthTokenId};
use rand::Rng;
...
impl SqliteScanStore {
    ...
    /// If `PRQNT_BOOTSTRAP_ADMIN_TOKEN` or legacy `PARQONAUT_BOOTSTRAP_TOKEN` is set, upserts label `bootstrap`.
    pub async fn bootstrap_auth_from_env(&self) -> Result<(), StoreError> {
        let raw = std::env::var("PRQNT_BOOTSTRAP_ADMIN_TOKEN")
            .or_else(|_| std::env::var("PARQONAUT_BOOTSTRAP_TOKEN"))
            .ok();
        let Some(raw) = raw.filter(|s| !s.is_empty()) else {
            return Ok(());
        };
        let role = match std::env::var("PARQONAUT_BOOTSTRAP_ROLE")
