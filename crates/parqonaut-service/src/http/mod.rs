//! HTTP server wiring (Axum). Handlers delegate to [`crate::service::ParqonautService`].
...
/// [`ParqonautService::execute_scan_and_persist`] path as synchronous scans.
///
...
pub fn build_router(service: ParqonautService) -> Router {
    build_router_with_workers(service, DEFAULT_WORKERS)
}

pub fn build_router_with_workers(service: ParqonautService, workers: usize) -> Router {
