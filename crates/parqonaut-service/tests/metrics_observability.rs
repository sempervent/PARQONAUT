...
use parqonaut_types::{AuthRole, JobId, TargetIdentity};
use serde_json::json;
use serial_test::serial;
use tower::ServiceExt;
use uuid::Uuid;

const TEST_BEARER_SECRET: &str = "parqonaut-http-test-token";
...
async fn metrics_endpoint_returns_prometheus_text() {
    ...
    assert!(
        ...
        "expected parqonaut metrics after one request; body:\n{body}"
    );
}
...
async fn worker_recovery_increments_jobs_requeued_total() {
    ...
    let service = ParqonautService::new(store);
