...
|------------|-----------|-----------|-----|------------------------|
| **Forensic scan / inspect** | `ScanEngine::run` local pipeline; parquet/CSV/JSON shallow probes; rules; findings; validated reports | `inspect` command (schema, row groups, metadata, optional stats) | Parquet input reader (arrow2); no diagnostic rules | `prqnt scan` via `paraclete-core`; `prqnt inspect` via `parqonaut-transform` |
...
| **CSV ↔ Parquet conversion** | None | None | Pipeline with bounded channel; CSV→CSV tested; CSV→Parquet needs writer completion | `prqnt convert` → `parqonaut-stream` |
...
| **CLI** | `paraclete` HTTP client | `parqknife` subcommands | `maw` positional args | `prqnt` unified hierarchy |
...
|--------|-------------|-----------------|
| Scan | `prqnt scan <path>` | `paraclete_core::ScanEngine::run` |
| Transform | `prqnt rewrite <in> <out> --compression zstd` | `parqonaut_transform` rewrite path |
| Stream | `prqnt convert <in.csv> -o <out.parquet>` | `parqonaut_stream` CSV→Parquet pipeline |
...
- Paraclete CLI remains HTTP-only; local scan exposed via new `prqnt scan`.
